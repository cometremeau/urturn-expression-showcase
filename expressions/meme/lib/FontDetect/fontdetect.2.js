 ...
 * 
 * @version 1.1  2012-04-12	Refined the algorithm to use fewer helper elements, more reference fonts,
 *								and quicker detection of a nonexistent font.
 * 
 * @version 1.2  2012-06-01	Added onFontLoaded for a callback to execute as soon as the font is 
...
fontdetect = function()
{
	...
	return	{
		...
		onFontLoaded: function (p_cssFontName, p_onLoad, p_onFail, p_options)
		{
			...
			if (this.isFontLoaded(p_cssFontName && p_onFail))
		...
		isFontLoaded: function (p_cssFontName)
		{
			...
			{
				$('#fontdetectHelper > SPAN').css('font-family', p_cssFontName + ',' + _aFallbackFonts[ix]);
				wThisFont = $('#fontdetectHelper > SPAN').width();
